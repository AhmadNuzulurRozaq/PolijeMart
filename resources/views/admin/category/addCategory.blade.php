...
<section class="max-w-2xl mx-auto px-2 sm:px-6 py-8 fade-in-up">
    <!-- Header Form -->
    <div class="mb-8">
        <h1 class="text-3xl font-black text-slate-900 tracking-tight bg-gradient-to-r from-primary-950 to-primary-750 bg-clip-text text-transparent uppercase">Tambah Kategori</h1>
        <p class="text-sm text-slate-500 mt-1.5 font-semibold flex items-center gap-1">
            ...
            <span>Tambahkan kategori baru untuk pengelompokan produk inventori.</span>
        </p>
    </div>

    <!-- Form Container -->
    <div class="bg-white p-6 sm:p-10 rounded-3xl shadow-premium border border-slate-100">
        <form action="{{ route('admin.storeCategory') }}" method="POST" id="submitForm" class="space-y-6">
            @csrf
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="text" name="kode_kategori" id="kodeKategori" value="{{ old('kode_kategori') }}" required placeholder="Masukkan kode kategori (misal: KTG-01)..."
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="text" name="nama_kategori" id="namaKategori" value="{{ old('nama_kategori') }}" required placeholder="Masukkan nama kategori baru..."
                    class="w-full p-3.5 bg-slate-50/50 border outline-none rounded-xl text-sm transition-all duration-300 font-semibold text-slate-800 placeholder-slate-400 focus:bg-white focus:ring-4 focus:ring-secondary-500/10
                        @error('nama_kategori')
                            border-red-500 focus:border-red-500
                        @else
